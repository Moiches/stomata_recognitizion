...
class StomataAnalyzer:
    def __init__(self,
        ...
        if use_unet:
            ...
            # Asegurar que el device está configurado correctamente
            if not hasattr(self.unet_segmenter, 'device'):
                import torch
                self.unet_segmenter.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
